        ...
        public class MonitoringController implements Initializable {
            ...
            @FXML private TableColumn<LogEntry, Void> qrColumn;
            ...
            private void setupTableColumns() {
                ...
                qrColumn.setCellFactory(param -> new TableCell<>() {
                    private final Button viewQrButton = new Button("🔍 View");
                    {
                        viewQrButton.getStyleClass().add("table-edit");
                        viewQrButton.setOnAction(e -> {
                            LogEntry entry = getTableView().getItems().get(getIndex());
                            if (entry != null) onViewQrCode(entry);
                        });
                    }
                    @Override
                    protected void updateItem(Void item, boolean empty) {
            ...
            private void onViewQrCode(LogEntry entry) {
                String qrPath = System.getProperty("user.home") + java.io.File.separator + "BYOD_QR_Codes"
                        + java.io.File.separator + "QR_" + entry.getFormId() + ".png";
                java.io.File qrFile = new java.io.File(qrPath);
                if (!qrFile.exists()) {
                    showAlert("QR Not Found", "No QR code file found for Form ID: " + entry.getFormId());
                    return;
                }
                Stage currentStage = (Stage) monitoringTableView.getScene().getWindow();
                com.example.monitoring.QRRegistrationSuccessWindow.show(currentStage, entry.getFormId(), qrPath);
